...
public class ServerChunkDataPacket implements Packet {
    ...
    public void read(Buffer.Input in) throws IOException {
        ...
        for (int i = 0; i < tileEntities.length; i++) {
            tileEntities[i] = mc.minecraft.Util.readNBT(in);
        }

        this.column = mc.minecraft.Util.readColumn(data, x, z, fullChunk, false, chunkMask, tileEntities);
    }
    ...
    public void write(Buffer.Output out) throws IOException {
        ...
        int mask = mc.minecraft.Util.writeColumn(netOut, this.column, this.column.hasBiomeData(), this.column.hasSkylight());
        ...
        for (Object tag : this.column.getTileEntities()) {
            mc.minecraft.Util.writeNBT(out, tag);
        }
    }
