...
import mc.minecraft.data.game.PlayerListEntry;
import mc.minecraft.data.game.PlayerListEntryAction;
...
public class ServerPlayerListEntryPacket implements Packet {
    ...
    public void read(Buffer.Input in) throws IOException {
        this.action = mc.minecraft.Magic.key(PlayerListEntryAction.class, in.readVarInt());
        ...
        for (int count = 0; count < this.entries.length; count++) {
            ...
            switch (this.action) {
                case ADD_PLAYER:
                    ...
                    int g = in.readVarInt();
                    GameMode gameMode = mc.minecraft.Magic.key(GameMode.class, g < 0 ? 0 : g);
                ...
                case UPDATE_GAMEMODE:
                    g = in.readVarInt();
                    GameMode mode = mc.minecraft.Magic.key(GameMode.class, g < 0 ? 0 : g);
        ...
        for (PlayerListEntry entry : this.entries) {
            ...
            switch (this.action) {
                case ADD_PLAYER:
                    ...
                    out.writeVarInt(mc.minecraft.Magic.value(Integer.class, entry.getGameMode()));
