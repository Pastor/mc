...
import mc.game.data.game.chunk.BlockStorage;
import mc.game.data.game.chunk.Chunk;
import mc.game.data.game.chunk.Column;
import mc.game.data.game.chunk.NibbleArray3d;
import mc.game.data.game.entity.metadata.*;
...
public final class Util {
    ...
    public static Rotation readRotation(Buffer.Input in) throws IOException {
        return new Rotation(in.readFloat(), in.readFloat(), in.readFloat());
    }

    public static void writeRotation(Buffer.Output out, Rotation rot) throws IOException {
    ...
    private static class NetInputStream extends InputStream {
        ...
        public NetInputStream(Buffer.Input in, byte firstByte) {
            this.in = in;
    ...
    private static class NetOutputStream extends OutputStream {
        private final Buffer.Output out;

        public NetOutputStream(Buffer.Output out) {
            this.out = out;
        }
