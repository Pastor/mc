...
abstract class TcpSession extends SimpleChannelInboundHandler<Packet> implements Session {
    ...
    static Session.Factory newFactory(Proxy proxy) {
        return new TcpSessionFactory(proxy);
    }

    private static final class TcpSessionFactory implements Session.Factory {
        private final Proxy proxy;

        private TcpSessionFactory(Proxy proxy) {
            this.proxy = proxy;
        }

        public Session newSession(Client client) {
            return new TcpClientSession(client.host(), client.port(), client.protocol(), client, proxy);
        }
    }
}
