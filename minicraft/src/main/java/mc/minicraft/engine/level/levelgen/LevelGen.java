package mc.minicraft.engine.level.levelgen;
...
public class LevelGen {
    ...
    public static byte[][] createAndValidateTopMap(int w, int h,
        ...
        do {
            ...
            if (count[Tile.rock.id & 0xff] < maxRock) continue;
            if (count[Tile.sand.id & 0xff] < maxSand) continue;
            if (count[Tile.grass.id & 0xff] < maxGrass) continue;
            if (count[Tile.tree.id & 0xff] < maxTree) continue;
            if (count[Tile.stairsDown.id & 0xff] < maxStairDown) continue;
    ...
    public static byte[][] createAndValidateUndergroundMap(int w, int h, int depth,
        ...
        do {
            ...
            if (count[Tile.rock.id & 0xff] < maxRock) continue;
            if (count[Tile.dirt.id & 0xff] < maxDirt) continue;
            if (count[(Tile.ironOre.id & 0xff) + depth - 1] < maxIronOre) continue;
            if (depth < 3) if (count[Tile.stairsDown.id & 0xff] < maxStairDown) continue;
    ...
    public static byte[][] createAndValidateSkyMap(int w, int h, int maxCloud, int maxStairDown) {
        int attempt = 0;
        do {
            ...
            if (count[Tile.cloud.id & 0xff] < maxCloud) continue;
            if (count[Tile.stairsDown.id & 0xff] < maxStairDown) continue;
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            byte[] map = LevelGen.createAndValidateTopMap(w, h, 100, 100, 100, 100, 2)[0];
//             byte[] map = LevelGen.createAndValidateUndergroundMap(w, h, (d++ % 3) + 1, 10, 10, 2, 2)[0];
//             byte[] map = LevelGen.createAndValidateSkyMap(w, h, 2000, 2)[0];
