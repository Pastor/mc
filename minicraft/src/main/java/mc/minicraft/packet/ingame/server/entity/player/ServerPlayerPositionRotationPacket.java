...
import mc.minicraft.data.game.entity.player.PositionElement;
...
public class ServerPlayerPositionRotationPacket implements Packet {
    ...
    public void read(Buffer.Input in) throws IOException {
        ...
        for (PositionElement element : PositionElement.values()) {
            int bit = 1 << mc.minicraft.Magic.value(Integer.class, element);
            if ((flags & bit) == bit) {
    ...
    public void write(Buffer.Output out) throws IOException {
        ...
        for (PositionElement element : this.relative) {
            flags |= 1 << mc.minicraft.Magic.value(Integer.class, element);
        }
