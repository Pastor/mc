...
import mc.minicraft.data.game.world.notify.*;
...
public class ServerNotifyClientPacket implements Packet {
    ...
    public void read(Buffer.Input in) throws IOException {
        this.notification = mc.minicraft.Magic.key(ClientNotification.class, in.readUnsignedByte());
        float value = in.readFloat();
        if (this.notification == ClientNotification.CHANGE_GAMEMODE) {
            this.value = mc.minicraft.Magic.key(GameMode.class, (int) value);
        } else if (this.notification == ClientNotification.DEMO_MESSAGE) {
            this.value = mc.minicraft.Magic.key(DemoMessageValue.class, (int) value);
        } else if (this.notification == ClientNotification.ENTER_CREDITS) {
            this.value = mc.minicraft.Magic.key(EnterCreditsValue.class, (int) value);
    ...
    public void write(Buffer.Output out) throws IOException {
        out.writeByte(mc.minicraft.Magic.value(Integer.class, this.notification));
        float value = 0;
        if (this.value instanceof Enum<?>) {
            value = mc.minicraft.Magic.value(Integer.class, (Enum<?>) this.value);
