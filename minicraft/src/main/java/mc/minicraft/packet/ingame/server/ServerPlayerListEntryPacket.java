...
import mc.minicraft.data.game.PlayerListEntry;
import mc.minicraft.data.game.PlayerListEntryAction;
...
public class ServerPlayerListEntryPacket implements Packet {
    ...
    public void read(Buffer.Input in) throws IOException {
        this.action = mc.minicraft.Magic.key(PlayerListEntryAction.class, in.readVarInt());
        ...
        for (int count = 0; count < this.entries.length; count++) {
            ...
            switch (this.action) {
                case ADD_PLAYER:
                    ...
                    int g = in.readVarInt();
                    GameMode gameMode = mc.minicraft.Magic.key(GameMode.class, g < 0 ? 0 : g);
                ...
                case UPDATE_GAMEMODE:
                    g = in.readVarInt();
                    GameMode mode = mc.minicraft.Magic.key(GameMode.class, g < 0 ? 0 : g);
        ...
        for (PlayerListEntry entry : this.entries) {
            ...
            switch (this.action) {
                case ADD_PLAYER:
                    ...
                    out.writeVarInt(mc.minicraft.Magic.value(Integer.class, entry.getGameMode()));
