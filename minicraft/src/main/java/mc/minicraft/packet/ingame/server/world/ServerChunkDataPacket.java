...
import mc.minicraft.data.game.chunk.Column;
...
public class ServerChunkDataPacket implements Packet {
    ...
    public void read(Buffer.Input in) throws IOException {
        ...
        for (int i = 0; i < tileEntities.length; i++) {
            tileEntities[i] = mc.minicraft.Util.readNBT(in);
        }

        this.column = mc.minicraft.Util.readColumn(data, x, z, fullChunk, false, chunkMask, tileEntities);
    }
    ...
    public void write(Buffer.Output out) throws IOException {
        ...
        int mask = mc.minicraft.Util.writeColumn(netOut, this.column, this.column.hasBiomeData(), this.column.hasSkylight());
        ...
        for (Object tag : this.column.getTileEntities()) {
            mc.minicraft.Util.writeNBT(out, tag);
        }
    }
